...
func NewDeployer(services *service.UseCase, operations *operation.Manager, tunnel TunnelPort, access AccessPort, dns DNSPort, connector ConnectorRuntime, origin OriginChecker) (*Deployer, error) {
	if services == nil || operations == nil || tunnel == nil || access == nil || dns == nil || connector == nil {
		return nil, errors.New("deployer dependencies are incomplete")
	}
...
func (d *Deployer) execute(ctx context.Context, op operation.Operation, item service.Service) error {
	...
	if err := d.waitConnector(ctx, item.ID); err != nil {
	...
	application, err := d.access.EnsureApplication(ctx, AccessApplicationSpec{ID: refs.AccessApplicationID, Name: item.Name, Domain: item.Hostname})
	if err != nil {
	...
	if err := d.services.SetRemoteRefs(ctx, item.ID, refs); err != nil {
		return fail(err, "service_state_unavailable", "DNS reference could not be saved")
	}
...
func (d *Deployer) waitConnector(ctx context.Context, serviceID string) error {
